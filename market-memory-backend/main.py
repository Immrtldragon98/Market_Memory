...
from app.api import alerts, analytics, journal, memory, visits, watchlist
from app.core.config import settings
from app.core.database import supabase
from app.services.coingecko import get_live_price

app = FastAPI(title="Market Memory API", version="1.0.0-rebuild")
...
def fetch_live_market_tick(symbol: str, asset_type: str, backend_id: str):
    if asset_type != "crypto":
        raise HTTPException(status_code=400, detail="Live price provider currently supports crypto assets only")
...
@app.get("/")
def health():
    return {"status": "running", "version": app.version, "product": "Market Memory"}
