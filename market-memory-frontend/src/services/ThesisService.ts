...
import { Thesis, ThesisCreateRequest } from '../types/contracts';
...
export const ThesisService = {
  create(payload: ThesisCreateRequest): Promise<Thesis> {
    return authorizedFetch('/api/theses', { method: 'POST', body: JSON.stringify(payload) });
  },
  list(symbol?: string): Promise<Thesis[]> {
    const query = symbol ? `?symbol=${encodeURIComponent(symbol)}` : '';
    return authorizedFetch(`/api/theses${query}`);
  },
  captureSnapshot(thesisId: number, marketPayload: Record<string, unknown>, note?: string) {
